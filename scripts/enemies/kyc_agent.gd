...
func _start_scan(target: Node2D):
	...
	# Create scan beam visual
	scan_beam = ColorRect.new()
	scan_beam.color = Color(1, 0.2, 0.2, 0.25)
	scan_beam.size = Vector2(scan_range, 6)
	scan_beam.global_position = global_position + Vector2(facing * 8, -24)
	...
	beam_tween.tween_property(scan_beam, "modulate:a", 0.4, 0.2)
	beam_tween.tween_property(scan_beam, "modulate:a", 1.0, 0.2)
...
func _scan_hit(target: Node2D):
	...
	# Show "FLAGGED" text on player
	var flag_lbl = Label.new()
	flag_lbl.text = "IDENTITY VERIFIED\nFLAGGED"
	flag_lbl.global_position = target.global_position + Vector2(-45, -75)
	flag_lbl.add_theme_font_size_override("font_size", 10)
	flag_lbl.add_theme_color_override("font_color", Color(1, 0.2, 0.2))
	flag_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	flag_lbl.size = Vector2(90, 30)
	flag_lbl.z_index = 500
	get_parent().add_child(flag_lbl)
	var tween = flag_lbl.create_tween()
	tween.tween_property(flag_lbl, "modulate:a", 0.0, 1.2)
	tween.tween_callback(flag_lbl.queue_free)

	# Screen flash red
	CombatJuice.hit_sparks(get_parent(), target.global_position + Vector2(0, -20), Color(1, 0.2, 0.2), 4)
	SFX.hit(get_tree())
